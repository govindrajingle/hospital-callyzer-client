...
async function loadHospitalData() {
    const [hospitalsResponse, usersResponse, rolesResponse, relationshipsResponse] = await Promise.all([getJson(ENDPOINTS.hospitals), getJson(ENDPOINTS.users), getJson(ENDPOINTS.roles), getJson(ENDPOINTS.relationships)]);
    ...
    return {
        ...
        relationships: unwrapArray(relationshipsResponse, ["relationships", "userRelationships"])
    };
}
...
function App() {
    const [data, setData] = useState({
        hospitals: [], roles: [], users: [], relationships: []
    });
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (normalized.hospitals.length) {
                setSelectedHospitalId((current) => normalized.hospitals.some((h) => String(h.id) === String(current)) ? current : normalized.hospitals[0].id);
            }

            if (normalized.users.length) {
                setSelectedUserId((current) => normalized.users.some((u) => String(u.id) === String(current)) ? current : normalized.users[0].id);
    ...
    const hospital = data.hospitals.find((item) => String(item.id) === String(selectedHospitalId));

    const roleById = useMemo(() => Object.fromEntries(data.roles.map((role) => [String(role.id), role])), [data.roles]);

    const userById = useMemo(() => Object.fromEntries(data.users.map((user) => [String(user.id), user])), [data.users]);
    ...
    const hospitalUsers = useMemo(() => data.users.filter((user) => !hospital || String(user.hospital_id) === String(hospital.id)), [data.users, hospital]);

    const hospitalRelationships = useMemo(() => data.relationships.filter((relationship) => !hospital || String(relationship.hospital_id) === String(hospital.id)), [data.relationships, hospital]);

    const rootUsers = useMemo(() => {
        const juniorIds = new Set(hospitalRelationships.map((relationship) => String(relationship.junior_user_id)));

        return hospitalUsers.filter((user) => !juniorIds.has(String(user.id)));
    }, [hospitalUsers, hospitalRelationships]);

    const selectedUser = selectedUserId != null ? userById[String(selectedUserId)] : null;
    ...
    return (<div className="app-shell">
        ...
        <main className="content">
            ...
            {hospital ? (<section className="hospital-card">
                ...
                {data.hospitals.length > 1 && (<select
                    className="hospital-select"
                    value={selectedHospitalId ?? ""}
                    onChange={(event) => setSelectedHospitalId(event.target.value)}
                >
                    {data.hospitals.map((item) => (<option key={item.id} value={item.id}>
                        {item.hospital_name}
                    </option>))}
                </select>)}
            ...
            </section>) : !loading ? (<div className="empty-page">No hospitals returned by the API.</div>) : null}
            ...
            <div className="grid">
                <section className="panel hierarchy-panel">
                    ...
                    <div className="tree">
                        {loading ? (<div className="tree-loading">Loading hierarchy…</div>) : hospital ? (
                            <div className="tree-root">
                                ...
                                <div className="tree-children">
                                    ...
                                    {!rootUsers.some((user) => isUserOrDescendantMatch(user, userById, childrenByUserId, roleById, search)) && search && (
                                        <div className="empty-tree">No matching users.</div>)}
                                </div>
                            </div>) : (<div className="empty-tree">No hierarchy data.</div>)}
                    </div>
                </section>
            ...
            <section className="panel roles-panel">
                ...
                <div className="role-grid">
                    {data.roles.map((role) => {
                        const parent = role.parent_role_id ? roleById[String(role.parent_role_id)] : null;

                        const count = hospitalUsers.filter((user) => String(user.role_id) === String(role.id)).length;
            ...
            <section className="panel relationships-panel">
                ...
                <div className="relationship-list">
                    {hospitalRelationships.map((relationship) => {
                        ...
                        return (<div className="relationship-row" key={relationship.id}>
...
function isUserMatch(user, roleById, search) {
    ...
    return (String(user.id).toLowerCase().includes(q) || user.full_name.toLowerCase().includes(q) || user.username.toLowerCase().includes(q) || role?.role_name?.toLowerCase().includes(q) || role?.role_code?.toLowerCase().includes(q));
}
...
function isUserOrDescendantMatch(user, userById, childrenByUserId, roleById, search) {
    ...
    return children.some((childId) => {
        ...
        return (child && isUserOrDescendantMatch(child, userById, childrenByUserId, roleById, search));
    });
}

function UserNode({
                      user, roleById, userById, childrenByUserId, expanded, toggle, selectedUserId, selectUser, search
                  }) {
    const role = roleById[String(user.role_id)];
    const children = (childrenByUserId[String(user.id)] || [])
    ...
    const visible = isUserOrDescendantMatch(user, userById, childrenByUserId, roleById, search);

    if (!visible) return null;

    return (<div className="tree-branch">
        <div
            className={`user-node ${String(selectedUserId) === String(user.id) ? "selected" : ""}`}
        ...
        >
            ...
            >
                {isExpanded ? "−" : "+"}
            </button>) : (<span className="expand-placeholder"/>)}
        ...
        {hasChildren && isExpanded && (<div className="nested-children">
            {children.map((child) => (<UserNode
                key={child.id}
                user={child}
                roleById={roleById}
                userById={userById}
...
function UserDetails({user, role, userById, roleById, relationships}) {
    const seniorRelations = relationships.filter((relationship) => String(relationship.junior_user_id) === String(user.id));

    const juniorRelations = relationships.filter((relationship) => String(relationship.senior_user_id) === String(user.id));
    ...
    return (<>
        ...
        <div className="relation-section">
            ...
            {seniorRelations.length ? seniorRelations.map((relationship) => {
                const senior = userById[String(relationship.senior_user_id)];
        ...
        <div className="relation-section">
            ...
            {juniorRelations.length ? juniorRelations.map((relationship) => {
                const junior = userById[String(relationship.junior_user_id)];
...
function RelationCard({type, user, role}) {
    return (<div className="relation-card">
...
function UserPill({user, role, onClick}) {
    return (<button className="user-pill" onClick={onClick}>
